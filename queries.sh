...
#the maximum value would always have been of a winner team
...
echo "$($PSQL "
select name from teams join games on teams.team_id = games.winner_id where year = 2014 and round = 'Eighth-Final'
union
select name from teams join games on teams.team_id = games.opponent_id where year = 2014 and round = 'Eighth-Final'
order by name")"
